...
> **Про Codex / ChatGPT- и Claude-подписки.** Подписочные квоты (например «Codex 80%»)
> **нельзя** подключить через AiGate: OpenAI и Anthropic не публикуют OAuth-flow и
> API остатка подписки для сторонних клиентов. Это осознанно оставлено как
> экспериментальная/внешне-зависимая часть (см. риск-реестр в плане). Что работает
> уже сейчас — их **API-ключи** (`api.openai.com`, `api.anthropic.com`) как обычные
> провайдеры с учётом usage и оценочной стоимости. OAuth-каркас в коде готов принять
> адаптер без изменения ядра, если/когда появится публичный flow.
