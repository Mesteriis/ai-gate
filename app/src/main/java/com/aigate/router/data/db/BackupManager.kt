...
class BackupManager(private val database: AppDatabase) {
    ...
    suspend fun importFromFile(file: File, password: String = ""): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            if (backupData.apiKeyEntriesJson.isNotBlank()) {
                try {
                    val keys = Json { ignoreUnknownKeys = true }.decodeFromString<List<com.aigate.router.service.ApiKeyEntry>>(backupData.apiKeyEntriesJson)
    ...
    /**
     * 获取备份文件目录（/sdcard/Download/AiGate/backup/）
     */
    fun getBackupDir(): File {
        val dir = File(android.os.Environment.getExternalStoragePublicDirectory(android.os.Environment.DIRECTORY_DOWNLOADS), "AiGate/backup")
        dir.mkdirs()
        return dir
    }
