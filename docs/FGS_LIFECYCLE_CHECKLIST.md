...
| 10 | Длинный SSE-стрим | не обрывается по таймауту раньше времени | ⏳ ручной QA (нужен live upstream) |
| 11 | Параллельные стримы | потокобезопасность session-кэшей (ConcurrentHashMap — из Фазы 6) | ⏳ ручной QA (нужен live upstream) |
...
Пункты **1–8 и 12 подтверждены** на устройстве `RFCY706SHEB` через adb-автоматизацию
(экран/Doze/энергосбережение/фоновый kill/force-stop/перезагрузка) и по коду. Boot-
автозапуск закрыт новым `service/BootReceiver` (использует ранее «мёртвое»
RECEIVE_BOOT_COMPLETED). Остаются только пункты 9–11 (Wi-Fi↔LTE, длинный SSE,
параллельные стримы) — им нужен живой upstream; это оставшийся ручной QA-долг v0.1.0,
